...
Only `IssuerWebhookSecret` is required — the PayFast, WorkOS, and Slack parameters all default to empty, which just runs the platform unmetered (both wallet funding and subscription billing stay on instant/free-tier sandbox behavior)/without SSO/without approval push notifications. Save whatever you pass for `IssuerWebhookSecret`; it's also what the sandbox purchase simulator and any real issuer integration need to sign webhooks with.
